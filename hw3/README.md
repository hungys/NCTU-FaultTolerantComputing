...
In this homework, we implement two kinds of broadcast protocol, which are *reliable broadcast* and *FIFO broadcast*. We leverage *perfect link* that we implemented in HW2, as the underlying communication link of broadcast protocol.

Since FIFO broadcast is based on reliable broadcast, we adopt the similar design concept as HW2 to implement each broadcast protocol as an actor. In this way, if an application want to use FIFO broadcast protocol, it will first spawn `FIFOBroadcast` actor, and behind the scene, a `ReliableBroadcast` actor is also created to serve as the underlying layer.

## Modifications to `FairlossLink` module

In Elixir, once the node is connected to another node, `Node.list` API returns not only the neighbor nodes, but also all the **reachable** nodes in the network. To simulate the link outage between nodes, we maintain a set which stores the link status to neighbors for each node's communication link. (See `links` set in `FairlossLink`)
...
As shown in the pseudo code, the same messages will only be delivered one time. Furthermore, it will be broadcasted again to the neighbors to make all the reachable nodes in the network be able to receive the broadcast message.
...
The `topo` command will call `neighbor` correspondingly for you to form a topology. The fully supported options are listed below,
...
To verify the correctness of `ReliableBroadcast`, we implement a validation method that can be called each time the message be delivered,
...
The message broadcasted by `bsrc` is inserted to a exclusive list `delivered[bsrc]`. The **sorted** list (i.e. `Enum.sort(delivered[bsrc])`) is passed into `md5` function to calculate a hash. Since `ReliableBroadcast` does not guarantee the FIFO-ordering of messages, the hash value calculated in this way must be identical across all the nodes.

For example, in a `ring3` network, we broadcast three messages in a row, and check if all final hash values are identical.
...
In the above example, we can see that no matter the messages are delivered in which order, the final md5 hash of the **sorted delivered list** are identical on all nodes. (i.e. `005c1f463a1c114edabe1d4dfd7dd789` in this example)

### `FIFOBroadcast` module

To verify the correctness of `ReliableBroadcast`, we implement another validation method that can be called each time the message be delivered,
...
The message broadcasted by `bsrc` is **concatenated** with a exclusive string `delivered[bsrc]`. Since `FIFOBroadcast` guarantees the FIFO-ordering of messages, the hash value of this exclusive string must be identical across all the nodes.
...
In the above example, we can see that the md5 hash of **concatenated messages** are identical on all nodes. (i.e. `005c1f463a1c114edabe1d4dfd7dd789` in this example)

## Performance

To run the performance test, you should initialize the node with special options. For exmaple, in `Hw3.init(:fifo, :exp, :false)`, the second parameter `:exp` indicates that the node are running in experiment mode, and the messages will not output to the console to reduce the standard I/O overhead; the third parameter `:false` indicates that the md5 validation will not be executed.

We also provide a useful command `perf <number>` to broadcast batch messages. For example, to broadcast 1000 messages (`Hello World` by default),
...
- reliable broadcast / FIFO broadcast
- send 1000/5000/10000 batch messages
- message output (standard I/O): **OFF**
- md5 validation: **OFF**
